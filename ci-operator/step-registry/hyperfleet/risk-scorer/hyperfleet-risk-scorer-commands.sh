...
# Generate a short-lived installation token via JWT exchange.
...
while true; do
    ...
    PR_FILES=$(python3 -c "import sys,json; a=json.loads(sys.argv[1]); b=json.loads(sys.argv[2]); print(json.dumps(a+b))" "${PR_FILES}" "${PAGE_DATA}")
...
# Remove stale risk labels before applying the new one
